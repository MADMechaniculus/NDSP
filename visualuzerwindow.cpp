    ...
    ui(new Ui::VisualuzerWindow)
{
    ...
    this->ui->plotter->axisRect()->setupFullAxesBox(true);
    this->ui->plotter->xAxis->setLabel("x");
    this->ui->plotter->yAxis->setLabel("y");

    // set up the QCPColorMap:
    QCPColorMap *colorMap = new QCPColorMap(this->ui->plotter->xAxis, this->ui->plotter->yAxis);
    int nx = 200;
    int ny = 200;
    ...
    QCPMarginGroup *marginGroup = new QCPMarginGroup(this->ui->plotter);
    this->ui->plotter->axisRect()->setMarginGroup(QCP::msBottom|QCP::msTop, marginGroup);
